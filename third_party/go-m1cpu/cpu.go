...
// PATCHED (local fork): added "&& !ios" to the build constraint. The IOKit
// constant used below (kIOMasterPortDefault, aliased from kIOMainPortDefault
// by the guard in the cgo preamble) is marked unavailable in the iPhoneOS SDK,
// so iOS builds fall back to the incompatible.go stubs. macOS is unaffected.
